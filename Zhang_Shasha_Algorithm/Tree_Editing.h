...
class Tree_Editing {
    public: 
        ...
        // Árvores e nós para processamento
        Tree* t1;
        Tree* t2;
        vector<Node*> nodes1;
        vector<Node*> nodes2;

        // Construtor atualizado para receber as árvores
        Tree_Editing(Tree* t1, Tree* t2);
         
        int tree_dist_calc(Tree T1, Tree T2);
          // Método para calcular a distância entre árvores/subárvores específicas
        int comput_tree_dist(int index1, int index2);
        
        // Métodos para acessar os nós das árvores 1 e 2
